...
export function tick(simState, policyState, config) {
    const { tick: currentTick, operativePolicies, stocks, flows } = simState;
    const { targets } = policyState;
    const { dt, defaultImplementationRate, decayRates } = config;

    // --- 1. Read ALL values from frozen incoming state ---
    const oldTaxRate = operativePolicies.taxRateTarget;
    const oldTaxIntegrity = operativePolicies.taxCollectionIntegrityTarget;
    const oldCustomsIntegrity = operativePolicies.customsIntegrityTarget;
    const oldHealthcareFunding = operativePolicies.healthcareFundingPctTarget;
    const oldProcurementFairness = operativePolicies.procurementFairnessTarget;
    const oldEducationFunding = operativePolicies.educationFundingPctTarget;
    const oldEducationIntegrity = operativePolicies.educationIntegrityTarget;
    const oldInfrastructureFunding = operativePolicies.infrastructureFundingPctTarget;
    const oldInfrastructureIntegrity = operativePolicies.infrastructureMaintenanceIntegrityTarget;
    const oldElectionIntegrity = operativePolicies.electionIntegrityTarget;
    const oldOversightFunding = operativePolicies.oversightFundingPctTarget;
    const oldOversightIndependence = operativePolicies.oversightIndependenceIntegrityTarget;
    const oldJudicialFunding = operativePolicies.judicialFundingPctTarget;
    const oldJudicialIntegrity = operativePolicies.judicialIndependenceIntegrityTarget;
    const oldRuleOfLawIndex = stocks.ruleOfLawIndex;
    const oldBrainDrainAccumulated = stocks.brainDrainAccumulated || 0.0;
    const oldDerived = simState.derived || {};

    // --- 2. Compute deltas and new operative policies ---
    ...
    // Leakage is derived from the incoming (old) integrity policy, dampened by rule of law
    ...
    // Add non-decay flows to the newly decayed stocks
    newStocks.treasuryBalance += treasuryDelta;
    newStocks.healthcareCapacity += currentHealthcareInvestmentRate * dt * config.healthcareInvestmentMultiplier;
    newStocks.educationCapacity += currentEducationInvestmentRate * dt * config.educationInvestmentMultiplier;
    newStocks.infrastructureQuality += currentInfrastructureInvestmentRate * dt * config.infrastructureInvestmentMultiplier;
    newStocks.oversightCapacity += currentOversightInvestmentRate * dt * config.oversightInvestmentMultiplier;
    newStocks.ruleOfLawIndex += currentJudicialInvestmentRate * dt * config.judicialInvestmentMultiplier;
    newStocks.brainDrainAccumulated = oldBrainDrainAccumulated + brainDrainRate * dt;
